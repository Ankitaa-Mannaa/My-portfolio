...
export default function LiquidEther({
  ...
  const resolvedColors = useMemo(() => {
    ...
    const fallback = [color0, color1, color2].filter(Boolean) as string[];
    return fallback.length ? fallback : defaultColors;
...
`;
    ...
    class ShaderPass {
      ...
      constructor(props: Record<string, unknown>) {
        this.props = props || {};
        this.uniforms = this.props.material
          ? (this.props.material as { uniforms: Uniforms }).uniforms
          : undefined;
      }
      init() {
    ...
    class Advection extends ShaderPass {
      ...
      override update(options?: { dt?: number; isBounce?: boolean; BFECC?: boolean }) {
    ...
    class Viscous extends ShaderPass {
      ...
      override update(options?: { viscous?: number; iterations?: number; dt?: number }) {
        if (!this.uniforms) return;
        let fbo_in: THREE.WebGLRenderTarget;
        let fbo_out: THREE.WebGLRenderTarget = this.props.output as THREE.WebGLRenderTarget;
    ...
    class Divergence extends ShaderPass {
      ...
      override update(options?: { vel?: THREE.WebGLRenderTarget }) {
    ...
    class Poisson extends ShaderPass {
      ...
      override update(options?: { iterations?: number }) {
        let p_in: THREE.WebGLRenderTarget;
        let p_out: THREE.WebGLRenderTarget = this.props.output as THREE.WebGLRenderTarget;
        const iter = options?.iterations ?? 0;
        for (let i = 0; i < iter; i++) {
          if (i % 2 === 0) {
    ...
    class Pressure extends ShaderPass {
      ...
      override update(options?: { vel?: THREE.WebGLRenderTarget; pressure?: THREE.WebGLRenderTarget }) {
